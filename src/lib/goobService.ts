...
export class GoobService {
    ...
    this.initPromise = new Promise((resolve) => {
      try {
        ...
        this.worker.onerror = (err) => {
          console.warn('GOOB WASM Worker error:', err);
        };
        ...
        console.error('GOOB WASM init error:', err);
        resolve();
      }
    });
  ...
  private handleMessage(line: string) {
    ...
    // Phase 1: Wait for readyok to ensure previous search is stopped
    if (!this.isSearching) {
  ...
  public async evaluatePosition(
    ...
    const job = this.evalQueue.then(() => {
      return new Promise<EngineEvaluation>((resolve) => {
        ...
        // Safety timeout: 15 seconds per move max
        setTimeout(() => {
...
export const goobService = new GoobService();
